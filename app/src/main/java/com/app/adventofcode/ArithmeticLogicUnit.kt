...
class ArithmeticLogicUnit(private val listItem: ArrayList<String>) {

    private fun List<String>.parse() = chunked(18).map { chunk ->
    ...
    private  fun calculateZ(w: Int, prevZ: Int, divZ: Int, addX: Int, addY: Int): Int {
        var z = prevZ
    ...
    private fun List<Triple<Int, Int, Int>>.calculateSerials(
    ...
    private fun List<Triple<Int, Int, Int>>.solve() = calculateSerials(calculateLimits(), 0, 0).map { it.toLong() }

    private fun List<Triple<Int, Int, Int>>.calculateLimits(): Array<Map<Int, Set<Int>>> {
    ...
    fun partOne()= listItem.parse().solve().maxOf { it }

    fun partTwo() = listItem.parse().solve().minOf { it }
}